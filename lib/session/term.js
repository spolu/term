...
var term = function(spec, my) {
  ...
  init = function() {
    ...
    vt.on('char_attr_reset', function() {
      my.cursor.attr = CHAR_ATTRS.NULL;
    });
    vt.on('char_attr_set_bold', function(state) {
      if(state) SET(my.cursor.attr, CHAR_ATTRS.BOLD);
      else UNSET(my.cursor.attr, CHAR_ATTRS.BOLD);
    });
    vt.on('char_attr_set_italic', function(state) {
      if(state) SET(my.cursor.attr, CHAR_ATTRS.ITALIC);
      else UNSET(my.cursor.attr, CHAR_ATTRS.ITALIC);
    });
    vt.on('char_attr_set_underline', function(state) {
      if(state) SET(my.cursor.attr, CHAR_ATTRS.UNDERLINE);
      else UNSET(my.cursor.attr, CHAR_ATTRS.UNDERLINE);
    });
    vt.on('char_attr_set_blink', function(state) {
      if(state) SET(my.cursor.attr, CHAR_ATTRS.BLINK);
      else UNSET(my.cursor.attr, CHAR_ATTRS.BLINK);
    });
    vt.on('char_attr_set_reverse', function(state) {
      if(state) SET(my.cursor.attr, CHAR_ATTRS.REVERSE);
      else UNSET(my.cursor.attr, CHAR_ATTRS.REVERSE);
    });
    ...
    vt.on('char_attr_set_foreground_index', function(idx) {
      if(idx) my.cursor.fg = require('../vt/color.js').color_palette[idx];
      else my.cursor.fg = '#fff';
    });
    vt.on('char_attr_set_background_index', function(idx) {
      if(idx) my.cursor.bg = require('../vt/color.js').color_palette[idx];
      else my.cursor.bg = '#000';
    });

    vt.on('char_attr_report_color_palette', function() {
      /* TODO: ignore for now */
    });
    vt.on('char_attr_set_color_palette', function(value) {
      /* TODO: ignore for now */
    });
