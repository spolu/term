...
var term = function(spec, my) {
  ...
  // @absolute {boolean} move ignoring scroll region
  // ```
  // Moves to the specified position clamping it if necessary
  //
  move_to = function(x, y, absolute) {
    if(IS_SET(my.cursor.state, CURSOR_STATE.ORIGIN) && !absolute)
      y += my.top;
  ...
  clear_region = function(x, y, cols. rows, char_value) {
    ...
    cols = cols || my.geometry[0];
    cols = common.clamp(cols, 0, my.geometry[0] - 1);
    rows = rows || my.geometry[1];
    rows = common.clamp(rows, 0, my.geometry[1] - 1);

    for(var i = my.base + x; i < my.base + x + rows; i ++) {
      for(var j = y; j < y + cols; j ++) {
        my.buffer[i][j] = glyph(char_value);
      }
    }
  };
  ...
  init = function() {
    ...
    vt.on('fill', function(char_value) {
      clear(0, 0, my.geometry[0], my.geometry[1], char_value);
    });
    ...
    vt.on('cursor_left', function(count) {
      move_to(my.cursor.x - count, my.cursor.y);
    });
    vt.on('cursor_down', function(count) {
      move_to(my.cursor.x, my.cursor.y + count);
    });
    vt.on('cursor_up', function(count) {
      move_to(my.cursor.x, my.cursor.y - count);
    });
    vt.on('cursor_right', function(count) {
      move_to(my.cursor.x + count, my.cursor.y);
    });
    ...
    vt.on('forward_tab_stop', function(count) {
      cout = count || 1;
      while(count--)
        my.cursor.x = next_stop();
    });
    vt.on('backward_tab_stop', function(count) {
      cout = count || 1;
      while(count--)
