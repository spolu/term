...
var term = function(spec, my) {
  ...
  resize = function(cols, rows, silent) {
    ...
    if(dirty.length > 0) {
      ...
      that.emit('refresh', my.dirty, slice);
      my.dirty = [];
    }
  };
  ...
  init = function() {
    ...
    my.vt.on('set_cursor_position', function(x, y) {
    ...
    /*                             ANSI MODES                                 */
    /**************************************************************************/
    my.vt.on('set_insert_mode', function(state) {
      if(state) SET(my.mode, TERM_MODE.INSERT);
      else UNSET(my.mode, TERM_MODE.INSERT);
    });
    my.vt.on('set_auto_carriage_return', function() {
    ...
    /*                             DEC MODES                                  */
    /**************************************************************************/
    my.vt.on('set_application_cursor', function(state) {
    ...
    my.vt.on('set_scroll_region', function(top, bottom) {
      if(top) my.scroll.top = common.clamp(0, term.geometry[1] - 1);
      if(bottom) my.scroll.bottom = common.clamp(0, term.geometry[1] - 1);
    });
    my.vt.on('set_reverse_video', function(state) {
      var m = my.mode;
    ...
    my.vt.on('set_application_keypad', function(state) {
