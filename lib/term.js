...
var term = function(spec, my) {
  ...
  //
  // ### cursor
  // Returns the cursor position adjusted within the buffer
  //
  cursor = function() {
    return {
      x: my.cursor.x,
      y: my.base + my.cursor.y
    };
  };
  ...
  init = function() {
    ...
    my.vt.on('set_alternate_mode', function(alt) {
      if(alt) {
        ...
        for(var i = my.base; i < my.buffer.length; i ++) {
          var l = [];
          my.buffer[i].forEach(function(glyph) {
            l.push(glyph.slice(0));
          });
          my.saved_screen.screen.push(l);
        }
        soft_reset();
        my.cursor = {
          attr: 256 | (257 << 9) | (CHAR_ATTRS.NULL << 18),
        ...
        if(my.saved_screen) {
          ...
          for(var i = 0; i < my.saved_screen.screen.length; i ++) {
            my.buffer[my.base + i] = my.saved_screen.screen[i];
          }
          /* Refresh */
          dirty(my.base + my.scroll.top);
          dirty(my.base + my.scroll.bottom);
