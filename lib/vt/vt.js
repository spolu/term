...
var vt = function(spec, my) {
    ...
    switch(code) {
      ...
      case '3': {
        if(my.allow_width_change) {
          that.emit('set_width', state ? 132 : 80);
          that.emit('clear_home');
          that.emit('set_vt_scroll_region', null, null);
        }
        break;
      }
      case: '5': {
      ...
      case '6': {
        that.emit('set_origin_mode', sate);
        break;
      }
      case '7': {
      ...
      case '40': {
        that.emit('set_allow_width_change', state);
        break;
      }
      case '45': {
      ...
      case '1011': {
        that.emit('set_scoll_on_keystroke', state);
        break;
      }
      case '1036': {
  ...
  my.ESC = {
    ...
    'Z': function() {
      that.emit('send_string', '\x1b[?1;2c');
    },
    ...
    '=': function() {
      that.emit('application_keypad', true);
    },
    // Normal Keypad (DECPNM)
    '>': function() {
      that.emit('application_keypad', false);
    },
  ...
  my.OSC = {
    ...
    '0': function() {
      that.emit('window_title', my.state.args()[0]);
    },
    // Change Window Title
    '2': function() {
      that.emit('window_title', my.state.args()[0]);
    ...
    '4': function() {
      ...
      for(var i = 0; i < n; i++) {
        ...
        var val = args[i*2+1];
        if(val === '?') {
          report_idxs.push(idx);
          continue;
        }
        val = require('./colors.js').x11_to_css(val);
        if(val) {
          that.emit('char_attr_set_color_palette', idx, val);
        }
      }
      if(report_idxs.length > 0) {
        that.emit('report_char_attr_color_palette', report_idxs);
      }
    },
  ...
  my.CSI = {
    ...
    '@': function() {
      that.emit('insert_space', my.state.int_arg(0, 1));
    },
    // Cursor Up (CUU)
    'A': function() {
    ...
    // Cursor Down (CUD)
    'B': function() {
      that.emit('cursor_down', my.state.int_args(0, 1));
    },
    // Cursor Forward (CUF)
    'C': function() {
      that.emit('cursor_right', my.state.int_args(0, 1));
    },
    // Cursor Backward (CUB)
    'C': function() {
      that.emit('cursor_left', my.state.int_args(0, 1));
    },
    ...
    'F': function() {
      that.emit('cursor_up', my.state.int_args(0, 1));
      that.emit('set_cursor_column', 0);
    },
    // Cursor Character Absolute (CHA)
    'G': function() {
      that.emit('set_cursor_column', my.state.int_args(0, 1) - 1);
    },
    // Cursor Position (CUP)
    'H': function() {
    ...
    'K': function(code) {
      ...
      if(!arg || arg === '0') {
        that.emit('erase_to_right');
      }
      else if(arg === '1') {
        that.emit('erase_to_left');
      }
      else if(arg === '2') {
    ...
    // Scroll Up (SU)
    'S': function() {
      that.emit('vt_scroll_up', my.state.int_arg(0, 1));
    },
    // Scroll Down (SD)
    'T': function() {
      if(my.state.args().length <= 1)
        that.emit('vt_scroll_down', my.state.int_arg(0, 1));
    },
    // Reset Title Mode Features
    '>T': ignore,
    // Erase Characters (ECH)
    'X': function() {
      that.emit('erase_to_right', my.state.int_arg(0, 1))
    },
    ...
    '>c': function() {
      that.emit('send_string', '\x1b[>0;256;0c');
    },
    // Line Position Absolute (VPA)
    'd': function() {
      that.emit('set_absolute_cursor_row', my.state.int_args(0, 1) - 1);
    },
    // Tab Clear (TBC)
    'g': function() {
      if(!my.state.args()[0] || my.state.args()[0] === '0') {
        that.emit('clear_tab_stop_at_cursor', false);
      }
    ...
    'm': function() {
      ...
      for (var i = 0; i < my.state.args().length; i++) {
        ...
        if(arg < 30) {
          ...
          if(arg === 7)
            that.emit('char_attr_set_invers', true);
          if(arg === 8)
          ...
          if(arg === 27)
            that.emit('char_attr_set_inverse', false);
          if(arg === 28)
    ...
    '?n': function() {
      if(my.state.args()[0] === '6') {
        ...
        that.emit('send_string', '\x1b[?11n');
      }
      else if(my.state.args()[0] === '25') {
        that.emit('send_string', '\x1b[?21n');
      }
      else if(my.state.args()[0] === '26') {
        that.emit('send_string', '\x1b[?12;1;0;0n');
      }
      else if(my.state.args()[0] === '53') {
        that.emit('send_string', '\x1b[?50n');
      }
    },
    // Hide Pointer
    '>p': ignore,
    ...
    'r': function() {
      ...
      that.emit('set_vt_scroll_region', scroll_top, scroll_bottom);
