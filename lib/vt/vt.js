...
var cursor_state = function(spec, my) {
  ...
  common.method(that, 'save', save, _super);
...
var vt = function(spec, my) {
  ...
  my.CSI = {
    ...
    // Cursor Down (CUD)
    'B': function() {
      that.emit('term_cursor_down', my.parse_state.int_args(0, 1));
    },
    // Cursor Forward (CUF)
    'C': function() {
      that.emit('term_cursor_right', my.parse_state.int_args(0, 1));
    },
    // Cursor Backward (CUB)
    'C': function() {
      that.emit('term_cursor_left', my.parse_state.int_args(0, 1));
    },
    ...
    'F': function() {
      that.emit('term_cursor_up', my.parse_state.int_args(0, 1));
      that.emit('term_set_cursor_column', 0);
    },
    // Cursor Character Absolute (CHA)
    'G': function() {
      that.emit('term_set_cursor_column', my.parse_state.int_args(0, 1) - 1);
    },
    // Cursor Position (CUP)
    'H': function() {
      that.emit('term_set_cursor_position', 
                my.parse_state.int_args(0, 1),
                my.parse_state.int_args(1, 1));
    },
    // Cursor Forward Tabulation (CHT)
    'I': function() {
      var count = my.parse_state.int_args(0, 1);
      /*
