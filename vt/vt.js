...
// PARSE_STATE
// -----------
// @class helper class to represent the parser state
//
// @param spec { def_fun }
// 
var parse_state = function(spec, my) {
  ...
  // @param buf optional value for buffer (defaults to null)
  ...
  // @param arg optional value for args[0]
  ...
  // @param buf optional value for buffer
  reset = function(buf) {
    that.reset_fun();
    that.reset_buf(buf);
    that.reset_args();
  ...
  // @param pos the argument number to retrieve
  // @param def_value the default value to return
  int_arg = function(pos, def_value) {
    var str = my.args[pos];
    if(str) {
  ...
  // @para count the number of bytes to advance
...
// CURSOR_STATE
// -----------
// @class helper class to store and revert a cursor state used in DECSC
//
// @param spec { vt }
// 
...
// VT
// --
// @class escape sequence interpreter 
//
...
var vt = function(spec, my) {
  ...
  // #### dispatch
  // Dispatch tothe function that handles the given CC1, ESC, CSI or VT52 code
