...
// CHAR_MAP
// --------
// @class Character map object.
//
// @param spec { name, glmap }
//
var char_map = function(spec, my) {
  ...
  reset = function(glmap) {
    ...
    var glkeys = Object.keys(my.glmap).map(function(key) {
      return '\\x' + fwk.zpad(key.charCodeAt(0).toString(16));
    });
    ...
    var grkeys = Object.keys(my.grmap).map(function(key) {
      return '\\x' + fwk.zpad(key.charCodeAt(0).toString(16), 2);
    });
    ...
    fwk.method(that, 'GL', GL, _super);

    var GR = function(str) {
      return str.replace(grre,
                         function(ch) { return my.grmap[ch] });
    };
    fwk.method(that, 'GR', GR, _super);
  };
